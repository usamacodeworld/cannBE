...
export class UpdatedSchemaCategory1750397792032 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        // Check if product_variants table exists before creating
        const productVariantsExists = await queryRunner.hasTable("product_variants");
        ...
        // Check if attribute_values table exists before creating
        ...
        // Check if attributes table exists before creating
        const attributesExists = await queryRunner.hasTable("attributes");
        if (!attributesExists) {
        ...
        if (categoriesTable) {
            const isParentColumn = categoriesTable.findColumnByName("isParent");
            if (!isParentColumn) {
                await queryRunner.query(`ALTER TABLE "categories" ADD "isParent" boolean DEFAULT false`);
            }

            const parentIdColumn = categoriesTable.findColumnByName("parent_id");
            if (!parentIdColumn) {
        ...
        const productVariantsTable = await queryRunner.getTable("product_variants");
        if (productVariantsTable) {
            const foreignKey = productVariantsTable.foreignKeys.find(fk => fk.columnNames.includes("productId"));
            if (!foreignKey) {
        ...
        const attributeValuesTable = await queryRunner.getTable("attribute_values");
        if (attributeValuesTable) {
            const foreignKey = attributeValuesTable.foreignKeys.find(fk => fk.columnNames.includes("attributeId"));
            if (!foreignKey) {
